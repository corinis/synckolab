	...
	while(cur != null)
	{
		...
		if (cur.nodeType == Node.ELEMENT_NODE)//1
		{
			switch (cur.nodeName.toUpperCase())
			{
				...
				case "EMAIL":
					this.tools.logMessage("email: " + email + " - " + getXmlResult(cur, "SMTP-ADDRESS", ""), this.global.LOG_ERROR + this.global.LOG_AB);
					switch (email)
					{
						case 0:
			  ...
			  case "BODY":
				  	...
				  	this.tools.logMessage("cur.firstchild.data.length="+cur.firstChild.data.length + " - cnotes=" + cnotes.length + " - card.notes=" + this.getCardProperty(card, "Notes").length , this.global.LOG_DEBUG + this.global.LOG_AB);
					found = true;
			  	break;
			  case "DEPARTMENT":
			  		if (cur.firstChild == null)
			...
			default:
		  		if (cur.firstChild == null)
		  			break;
				this.tools.logMessage("XC FIELD not found: " + cur.nodeName + ":" + decode4XML(cur.firstChild.data), this.global.LOG_WARNING + this.global.LOG_AB);
...
com.synckolab.addressbookTools.list2Xml = function(card, fields) {
	...
	xml += nodeWithContent("categories", this.getCardProperty(card, "Category"), false);
	...
	if (checkExist(this.getCardProperty(card, "Description")))
			xml +=" <body>"+encode4XML(this.getCardProperty(card, "Description"))+"</body>\n";
	if (checkExist(this.getCardProperty(card, "Notes")))
			xml +=" <body>"+encode4XML(this.getCardProperty(card, "Notes"))+"</body>\n";
	xml += nodeWithContent("display-name", this.getCardProperty(card, "ListNickName"), false);
	...
	if (cList.addressLists)
 	{
		...
		if (total)
		{
			for ( var i = 0;  i < total; i++ )
			{
				...
				xml += "  <member>";
				xml += nodeWithContent("display-name", this.getCardProperty(cur, "DisplayName"), false);		
				if (checkExist(this.getCardProperty(card, "PrimaryEmail")))						
					xml += nodeWithContent("smtp-address", cur.primaryEmail, false);
				else
				if (checkExist(this.getCardProperty(card, "SecondEmail")))
					xml += nodeWithContent("smtp-address", cur.secondEmail, false);
				else
					this.tools.logMessage("ERROR: List entry without an email?!?" + getUID(cur), this.global.LOG_ERROR + this.global.LOG_AB);
...
com.synckolab.addressbookTools.list2Vcard = function(card, fields) {
	...
	// N:Lastname;Firstname;Other;Prefix;Suffix
 	if (checkExist (this.getCardProperty(card, "FirstName")) || checkExist (this.getCardProperty(card, "LastName")))
		msg += "N:" + this.getCardProperty(card, "LastName") + ";" + this.getCardProperty(card, "FirstName") + ";;;\n";
 	if (checkExist (this.getCardProperty(card, "DisplayName")))
		msg += "FN:" + this.getCardProperty(card, "DisplayName") + "\n";
 	if (checkExist (this.getCardProperty(card, "ListNickName")))
		msg += "FN:" + this.getCardProperty(card, "ListNickName") + "\n";
 	if (checkExist (this.getCardProperty(card, "NickName")))
		msg += "NICKNAME:" + this.getCardProperty(card, "NickName") + "\n";
 	if (checkExist (this.getCardProperty(card, "JobTitle")))
		msg += "TITLE:" + this.getCardProperty(card, "JobTitle") + "\n";
	if (checkExist (this.getCardProperty(card, "Company")))
 	...
 	if (checkExist (this.getCardProperty(card, "PreferMailFormat"))) { 
 	...
 	if (checkExist (this.getCardProperty(card, "AimScreenName"))) 
		msg += "X-AIM:" + this.getCardProperty(card, "AimScreenName") + "\n"; 
  	if (checkExist (this.getCardProperty(card, "CellularNumber")))
		msg += "TEL;TYPE=CELL:" + this.getCardProperty(card, "CellularNumber") + "\n";
 	if (checkExist (this.getCardProperty(card, "HomePhone")))
		msg += "TEL;TYPE=HOME:" + this.getCardProperty(card, "HomePhone") + "\n";
 	if (checkExist (this.getCardProperty(card, "FaxNumber")))
		msg += "TEL;TYPE=FAX:" + this.getCardProperty(card, "FaxNumber") + "\n";
 	if (checkExist (this.getCardProperty(card, "WorkPhone")))
		msg += "TEL;TYPE=WORK:" + this.getCardProperty(card, "WorkPhone") + "\n";
 	if (checkExist (this.getCardProperty(card, "PagerNumber")))
		msg += "TEL;TYPE=PAGER:" + this.getCardProperty(card, "PagerNumber") + "\n";
 	if (checkExist (this.getCardProperty(card, "Department")))
		msg += "DEPT:" + this.getCardProperty(card, "Department") + "\n";
	// BDAY:1987-09-27T08:30:00-06:00
	if (checkExist(this.getCardProperty(card, "BirthYear")) 
		||checkExist(this.getCardProperty(card, "BirthDay")) 
		|| checkExist(this.getCardProperty(card, "BirthMonth")))
	{
		msg += "BDAY:";
	...
	if (checkExist(this.getCardProperty(card, "AnniversaryYear")) 
		||checkExist(this.getCardProperty(card, "AnniversaryDay")) 
		||checkExist(this.getCardProperty(card, "AnniversaryMonth")))
	{
		msg += "ANNIVERSARY:" ;
	...
	if (checkExist (this.getCardProperty(card, "WorkAddress2")) 
		|| checkExist (this.getCardProperty(card, "WorkAddress")) 
		|| checkExist (this.getCardProperty(card, "WorkCountry")) 
		|| checkExist (this.getCardProperty(card, "WorkCity")) 
		|| checkExist (this.getCardProperty(card, "WorkState")))
	{
		msg += "ADR;TYPE=WORK:;";
	...
	// ADR:POBox;Ext. Address;Address;City;State;Zip Code;Country
	if (checkExist (this.getCardProperty(card, "HomeAddress2")) 
		|| checkExist (this.getCardProperty(card, "HomeAddress")) 
		|| checkExist (this.getCardProperty(card, "HomeCountry")) 
		|| checkExist (this.getCardProperty(card, "HomeCity")) 
		|| checkExist (this.getCardProperty(card, "HomeState")))
	{
		msg += "ADR;TYPE=HOME:;";
 	...
 	if (checkExist (this.getCardProperty(card, "Custom1")))
		msg += "CUSTOM1:" + this.getCardProperty(card, "Custom1").replace (/\n/g, "\\n") + "\n";
 	if (checkExist (this.getCardProperty(card, "Custom2")))
		msg += "CUSTOM2:" + this.getCardProperty(card, "Custom2").replace (/\n/g, "\\n") + "\n";
 	if (checkExist (this.getCardProperty(card, "Custom3")))
	...
	if (cList.addressLists)
 	{
		var total = cList.addressLists.Count();
		this.tools.logMessage ("List has " + total + " contacts", this.global.LOG_INFO + this.global.LOG_AB);
		if (!total || total == 0)
...
com.synckolab.addressbookTools.card2Xml = function(card, fields) {
	...
	if (checkExist (this.getCardProperty(card, "FirstName")) || checkExist (this.getCardProperty(card, "LastName")) ||checkExist (this.getCardProperty(card, "DisplayName")) ||
		checkExist (this.getCardProperty(card, "NickName")))
	{
		xml += " <name>\n";
		if (checkExist (this.getCardProperty(card, "FirstName")))
...
//			xml += "  <middle-names>"+this.getCardProperty(card, "NickName")+"</middle-names>\n"; // not really correct...
		if (checkExist (this.getCardProperty(card, "LastName")))
	...
	xml += nodeWithContent("organization", this.getCardProperty(card, "Company"), false);
	xml += nodeWithContent("web-page", this.getCardProperty(card, "WebPage1"), false);
	// not really kolab.. but we need that somewhere
	xml += nodeWithContent("business-web-page", this.getCardProperty(card, "WebPage2"), false);
	xml += nodeWithContent("im-address", this.getCardProperty(card, "AimScreenName"), false);
	xml += nodeWithContent("department", this.getCardProperty(card, "Department"), false);
//" <office-location>zuhaus</office-location>\n";
//" <profession>programmierer</profession>\n";
	xml += nodeWithContent("job-title", this.getCardProperty(card, "JobTitle"), false);
	xml += nodeWithContent("nick-name", this.getCardProperty(card, "NickName"), false);
	...
	if (checkExist(this.getCardProperty(card, "HomeAddress")) || checkExist(this.getCardProperty(card, "HomeAddress2")) ||
		checkExist(this.getCardProperty(card, "HomeCity")) || checkExist(this.getCardProperty(card, "HomeState")) ||
		checkExist(this.getCardProperty(card, "HomeZipCode")) || checkExist(this.getCardProperty(card, "HomeCountry")))
	{
		xml += " <address>\n";
		xml += "  <type>home</type>\n";
		xml += nodeWithContent("street", this.getCardProperty(card, "HomeAddress"), false);
		xml += nodeWithContent("street2", this.getCardProperty(card, "HomeAddress2"), false);
		xml += nodeWithContent("locality", this.getCardProperty(card, "HomeCity"), false);
		xml += nodeWithContent("region", this.getCardProperty(card, "HomeState"), false);
		xml += nodeWithContent("postal-code", this.getCardProperty(card, "HomeZipCode"), false);
		xml += nodeWithContent("country", this.getCardProperty(card, "HomeCountry"), false);
		xml += " </address>\n";
	}

	if (checkExist(this.getCardProperty(card, "WorkAddress")) || checkExist(this.getCardProperty(card, "WorkAddress2")) ||
		checkExist(this.getCardProperty(card, "WorkCity")) || checkExist(this.getCardProperty(card, "WorkState")) ||
		checkExist(this.getCardProperty(card, "WorkZipCode")) || checkExist(this.getCardProperty(card, "WorkCountry")))
	{
		xml += " <address>\n";
		xml += "  <type>business</type>\n";
		xml += nodeWithContent("street", this.getCardProperty(card, "WorkAddress"), false);
		xml += nodeWithContent("street2", this.getCardProperty(card, "WorkAddress2"), false);
		xml += nodeWithContent("locality", this.getCardProperty(card, "WorkCity"), false);
		xml += nodeWithContent("region", this.getCardProperty(card, "WorkState"), false);
		xml += nodeWithContent("postal-code", this.getCardProperty(card, "WorkZipCode"), false);
		xml += nodeWithContent("country", this.getCardProperty(card, "WorkCountry"), false);
		xml += " </address>\n";
	}
		
	//xml += nodeWithContent("preferred-address", this.getCardProperty(card, "DefaultAddress"), false); @deprecated
	xml += nodeWithContent("custom1", this.getCardProperty(card, "Custom1"), false);
	xml += nodeWithContent("custom2", this.getCardProperty(card, "Custom2"), false);
	xml += nodeWithContent("custom3", this.getCardProperty(card, "Custom3"), false);
...
com.synckolab.addressbookTools.equalsContact = function(a, b) {
	...
	for(var i=0 ; i < fieldsArray.length ; i++ ) {
		...
		if (sa.length != sb.length || sa != sb)
		{
			...
			this.tools.logMessage ("not equals " + fieldsArray[i] + " '" + sa + "' vs. '" + sb + "'", this.global.LOG_DEBUG + this.global.LOG_AB);
			return false;
		}
	}
...
com.synckolab.addressbookTools.parseMessage = function(message, extraFields, cards) {
	...
	{
		this.tools.logMessage("XML message!", this.global.LOG_INFO + this.global.LOG_AB);	
		return xml2Card(message, extraFields, cards);
	}
	else
		this.tools.logMessage("VCARD/VLIST!", this.global.LOG_INFO + this.global.LOG_AB);	
	...
	for (var i = 0; i < lines.length; i++)
	{
		...
		{
			this.tools.logMessage("parsing a list: " + message, this.global.LOG_DEBUG + this.global.LOG_AB);	
	...
	{
		this.tools.logMessage("unparseable: " + message, this.global.LOG_ERROR + this.global.LOG_AB);	
		return null;
	}
	return newCard;
};
	...
	for (var i = startI; i < lines.length && i < endI; i++)
	{
		...
		switch (tok[0].toUpperCase())
		{
			...
			case "EMAIL": //This is here to limit compact to existing vcards
				...
				{
					this.tools.logMessage("additional email found: " + tok[1], this.global.LOG_WARNING + this.global.LOG_AB);	
					addField(extraFields, tok[0], tok[1]);
				}
		  ...
		  default:
			this.tools.logMessage("VC FIELD not found: " + tok[0] + ":" + tok[1], this.global.LOG_WARNING + this.global.LOG_AB);
		  	addField(extraFields, tok[0], tok[1]);
		  	break;
		} // end switch
	}
...
com.synckolab.addressbookTools.card2Human = function(card) {
	...
	if (checkExist (this.getCardProperty(card, "Company")))
 	...
 	if (checkExist (this.getCardProperty(card, "AhomePhone")))
 ...
 */
com.synckolab.addressbookTools.card2Message = function(card, email, format, fFile) {
	...
	this.tools.logMessage("creating message out of card... ", this.global.LOG_INFO + this.global.LOG_AB);
 ...
 */
com.synckolab.addressbookTools.getUID = function(card) {
	if (card == null)
		return null;
 ...
 */
com.synckolab.addressbookTools.getCardProperty = function(card, prop) {
	// tbird 3
	if (card.getProperty)
	{
...
com.synckolab.addressbookTools.setCardProperty = function(card, prop, value) {
	...
	else
	{
		...
		switch (prop)
		{
			...
			default:
				this.tools.logMessage("Unable to set property: " + prop + " (reason: not found): " + value, this.global.LOG_WARNING + this.global.LOG_AB);
				return;
		}
	}
};
